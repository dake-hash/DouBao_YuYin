...
| 组件 | 选型 | 用途 | 备注 |
|------|------|------|------|
| 采集 | PyAudioWPatch | 麦克风录音 | pyaudio 的 Windows 增强版，有预编译 wheel |
...
| 窗口管理 | ctypes (windll.user32) | 获取活动窗口句柄 | 注入文本前确认目标窗口，无需 pywin32 |
...
| 内存 | 4GB+ |
| 硬盘 | 500MB 可用空间（venv 安装后约 450MB，含 PySide6 瘦身后）|

...
### 验收标准
- [ ] 运行 `python src/main.py` 不报错
- [ ] 系统托盘出现图标
- [ ] 右键托盘图标 → 点击「退出」→ 程序正常退出
- [ ] 项目根目录生成 `settings.json` 文件
...
### 验收标准
- [ ] 点击桌宠弹出菜单
- [ ] 点击「开启语音识别」→ settings.json 中 voice_enabled 变为 true
- [ ] 再次点击桌宠 → 菜单显示「关闭语音识别」
- [ ] 开启后桌宠外观有明显变化（绿点/高亮边框/不同动画）
- [ ] 关闭程序重新打开 → 开关状态保持（读取 settings.json）
